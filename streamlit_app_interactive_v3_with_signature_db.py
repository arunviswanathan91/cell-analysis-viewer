...
def organize_signatures_by_compartment():
    """Organize all signatures by compartment and cell type"""
    signatures = load_signatures()
    
    # Map cell types to compartments based on available data
    compartment_mapping = {
        'Immune Fine': [],
        'Immune Coarse': [],
        'Non-Immune': []
    ...
    # Load available cell types from each compartment
    for comp in ['Immune Fine', 'Immune Coarse', 'Non-Immune']:
        cells = get_available_cells(comp)
        compartment_mapping[comp] = cells
    ...
    for sig_entry in signatures:
        cell_type = sig_entry['cell_type']
        
        # Find which compartment this cell type belongs to
        for comp, cells in compartment_mapping.items():
            # Normalize for comparison
            normalized_cells = [c.upper().replace('_', ' ') for c in cells]
            normalized_cell_type = cell_type.upper().replace('_', ' ')
            
            if normalized_cell_type in normalized_cells:
                if cell_type not in organized[comp]:
                    organized[comp][cell_type] = []
                organized[comp][cell_type].append(sig_entry)
                break
    
    return organized

def plot_signature_count_dashboard():
    ...
    for idx, comp in enumerate(compartments):
        ...
        # Sort cell types by signature count
        cell_types = sorted(comp_data.keys(), 
                          key=lambda x: len(comp_data[x]), 
                          reverse=True)
        
        signature_counts = [len(comp_data[ct]) for ct in cell_types]
        cell_type_display = [ct.replace('_', ' ').title() for ct in cell_types]
...
def create_signature_table(cell_type_signatures):
    """Create an interactive display of signatures with gene lists"""
    if not cell_type_signatures:
        return None
    
    # Create structured data for display
    table_data = []
    for sig in cell_type_signatures:
        table_data.append({
            'Signature': sig['signature'].replace('_', ' '),
            'Gene Count': len(sig['genes']),
            'Genes': ', '.join(sorted(sig['genes']))
        })
    
    df = pd.DataFrame(table_data)
    df = df.sort_values('Gene Count', ascending=False).reset_index(drop=True)
...
def main():
    ...
    **Genes:** {len(genes)}
    ...
    if generate:
        ...
        # Tab 6: Signature Database Explorer
        with tabs[5]:
            st.markdown("### 📚 Explore Signature Database")
            
            st.markdown("""
            <div class="info-box">
            <b>🔍 Comprehensive Signature Database Overview</b><br>
            Explore all gene signatures organized by compartment and cell type. This database contains metabolic and 
            functional gene signatures used to characterize the tumor microenvironment.
            </div>
            """, unsafe_allow_html=True)
            
            # Dashboard Section
            st.markdown("#### 📊 Signature Count Dashboard")
            st.markdown("Overview of signature distribution across compartments and cell types")
            
            with st.spinner("Generating signature count dashboard..."):
                try:
                    fig_dashboard = plot_signature_count_dashboard()
                    if fig_dashboard:
            ...
            st.markdown("#### 🔎 Signature Details Explorer")
            st.markdown("Select a compartment and cell type to view all signatures and their gene compositions")
            ...
            explorer_cells = get_available_cells(explorer_comp)
            
            with col2:
                if explorer_cells:
                    explorer_cell_display = {cell.replace('_', ' ').title(): cell for cell in explorer_cells}
            ...
            if selected_explorer_cell:
                cell_signatures = get_cell_signatures(selected_explorer_cell)
                
                if cell_signatures:
                    st.markdown(f"""
                    <div class="method-box">
                    <b>📋 {selected_explorer_cell_display}</b> in <b>{explorer_comp}</b><br>
                    Total Signatures: <b>{len(cell_signatures)}</b>
                    </div>
                    """, unsafe_allow_html=True)
                    
                    # Create and display signature table
                    sig_df = create_signature_table(cell_signatures)
                    
                    if sig_df is not None:
                        st.markdown("##### 📑 Signature Details")
                        
                        # Display as expandable sections for better readability
                        for idx, row in sig_df.iterrows():
                            with st.expander(f"**{row['Signature']}** - {row['Gene Count']} genes"):
                                st.markdown(f"**Number of genes:** {row['Gene Count']}")
                                st.markdown("**Gene list:**")
                                
                                # Display genes in a more readable format (columns)
                                genes_list = row['Genes'].split(', ')
                                
                                # Create columns for gene display
                                n_cols = 4
                                cols = st.columns(n_cols)
                                for i, gene in enumerate(genes_list):
                                    col_idx = i % n_cols
                                    cols[col_idx].markdown(f"- {gene}")
                        
                        # Also show summary table
                        st.markdown("---")
                        st.markdown("##### 📊 Summary Table")
                        st.dataframe(
                            sig_df[['Signature', 'Gene Count']],
                        ...
                        st.download_button(
                            label="📥 Download Signature Data (CSV)",
                            data=csv,
                            file_name=f"{selected_explorer_cell}_{explorer_comp.replace(' ', '_')}_signatures.csv",
                            mime="text/csv"
                        )
                else:
                    st.warning(f"No signatures found for {selected_explorer_cell_display}")
